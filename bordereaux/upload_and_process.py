"""Truebind: Upload & Process page -- upload (single sheet or multi-sheet
workbook) -> review mapping per sheet (with a Lloyd's/saved-template
suggestion badge) -> validation/duplicates -> download, with the run
persisted (report, exceptions, leakage flags, audit trail) so the
Leakage & Duplicates and Compliance & Audit pages can see it afterwards.
"""
...
from bordereaux import audit, leakage, persistence, pipeline, templates, theme  # noqa: E402
from bordereaux.lloyds_template import seed_lloyds_template  # noqa: E402
from bordereaux.mapping import ai_mapping_available, audit_trail  # noqa: E402
...
ACTOR = "streamlit_user"  # no multi-user auth yet -- see plan's Streamlit-viability note
...
seed_lloyds_template(db_session)
db_session.commit()
...
st.caption(
    ...
    "workbook. We standardise it, check it for errors, duplicates and payment leakage, and "
    "hand back a clean export plus a one-page data-quality report — no integration required."
)
...
with st.expander("What happens under the hood? (plain English)"):
    st.write(
        ...
        "the ten core fields insurers care about, and asks you to confirm that mapping "
        "before touching any data — a wrong mapping is never applied silently, and a column "
        "nobody could map is shown as not found rather than quietly treated as blank. It "
        "then checks every row for the kind of errors that creep into bordereaux, looks for "
        "claims reported more than once, and flags payments that may have leaked out twice "
        "for the same payee. Nothing is changed, merged or deleted automatically: everything "
        "is flagged for a human to review, and every mapping decision and export is logged "
        "to an audit trail. The tool prepares data for human review; it does not make any "
        "claims decisions itself."
    )
...
sheets = pipeline.load_workbook(source_path, display_name=source_name)
...
for proposal in proposals:
    ...
    with st.expander(label, expanded=bool(unmapped_required) or len(active_sheets) == 1):
        template_suggestion = templates.suggest_template(db_session, list(sheet.raw.columns))
        use_template_key = f"use_template_{sheet.sheet_name}"
        if template_suggestion:
            matched_template, score, template_field_map = template_suggestion
            st.info(f"📋 Suggested template: **{matched_template.name}** "
                    f"({score * 100:.0f}% of this sheet's columns match) — "
                    f"accept to pre-fill the mapping below, or review/adjust as normal.")
            st.checkbox(f"Use '{matched_template.name}' to pre-fill this sheet's mapping",
                        key=use_template_key, value=st.session_state.get(use_template_key, False))
        else:
            template_field_map = {}

        use_template = st.session_state.get(use_template_key, False)
        ...
        for col in sheet.raw.columns:
            ...
            if use_template and col in template_field_map:
                default_code = template_field_map[col]
...
# --- persistence: report, mapping audit trail, exceptions, leakage flags ---
if st.session_state.get("persisted_for") != source_name:
    db_report = persistence.persist_report(db_session, h, source_name, uploaded_by=ACTOR)
    for proposal in proposals:
        trail = audit_trail(proposal.mapping.suggestions,
                             st.session_state["confirmed_mappings"].get(proposal.sheet.sheet_name, {}))
        audit.log_mapping_decisions(db_session, ACTOR, db_report.id, trail)
    persistence.persist_exceptions(db_session, db_report.id, result.canonical, result.validation_result.exceptions)
    leakage_flags_df = leakage.find_leakage(result.canonical)
    persistence.persist_leakage_flags(db_session, db_report.id, result.canonical, leakage_flags_df)
    db_session.commit()
    st.session_state["persisted_for"] = source_name
    st.session_state["current_report_id"] = db_report.id
    st.session_state["current_leakage_count"] = len(leakage_flags_df)

st.caption(f"✅ This run is saved (report `{st.session_state['current_report_id'][:8]}…`) — see it "
           f"on the Leakage & Duplicates and Compliance & Audit pages any time.")
...
with c1:
    theme.render_stat("Overall grade", f"{h.grade} / 5")
    st.markdown(theme.grade_badge(h.grade, h.grade_label), unsafe_allow_html=True)
with c2:
    theme.render_stat("Avg. field completeness", f"{h.overall_completeness_pct:.0f}%")
with c3:
    theme.render_stat("Rows with an exception", f"{h.exception_rate_pct:.0f}%")
with c4:
    theme.render_stat("Rows flagged as duplicates", f"{h.duplicate_rate_pct:.0f}%")

if st.session_state.get("current_leakage_count"):
    st.markdown(
        f"💰 **{st.session_state['current_leakage_count']} payment-leakage flag(s)** raised on this run — "
        f"see the **Leakage & Duplicates** page for the exposure estimate and details.",
    )
...
with tab1:
    ...
    if exceptions.empty:
        ...
        badge_map = {
            "arithmetic_mismatch": theme.arithmetic_state_badge("mismatch", "MISMATCH"),
            "missing_mandatory_field": theme.mapping_state_badge("unmapped", "MISSING FIELD"),
        }
        st.markdown(
            " ".join(dict.fromkeys(
                badge_map.get(r, theme.render_status_badge(r, "#374151", "#F3F4F6"))
                for r in exceptions["rule"]
            )),
    ...
    if result.validation_result.arithmetic_not_evaluable_count:
        st.markdown(
            theme.arithmetic_state_badge("not_evaluable", "NOT EVALUABLE")
            + f" &nbsp; {result.validation_result.arithmetic_not_evaluable_count} row(s) could not be "
...
with d1:
    if st.download_button(
        ...
        audit.log_action(db_session, ACTOR, audit.ACTION_EXPORT_TRIGGERED, entity_type="segregated_export",
...
with d2:
    if st.download_button(
        ...
        audit.log_action(db_session, ACTOR, audit.ACTION_EXPORT_TRIGGERED, entity_type="health_report_xlsx",
                          report_id=st.session_state.get("current_report_id"))
        db_session.commit()
with d3:
    if st.download_button(
        ...
        audit.log_action(db_session, ACTOR, audit.ACTION_EXPORT_TRIGGERED, entity_type="health_report_pdf",
                          report_id=st.session_state.get("current_report_id"))
        db_session.commit()
